...
def preprocess_data(input_filepaths, char2idx, output_dirpath):
    ...
    for fp in input_filepaths:
        ...
        questions_padded = np.array(
            [q + [0] * (160 - len(q)) for q in questions_encoded]
        )
        answers_padded = np.array(
            [[1] + a + [2] + [0] * (30 - len(a)) for a in answers_encoded]
        )
...
def get_paired_filepaths(filepaths):
    ...
    for fp in filepaths:
        ...
        if identifier not in identifier_dict:
            ...
            identifier_dict[identifier] = tuple(
                sorted(identifier_dict[identifier], reverse=True)
            )
...
def load_train(mode, num_files_to_include=None, verbose=True):
    ...
    for questions_filepath, answers_filepath in tqdm(paired_filepaths):
        ...
        module_name_to_arrays[module_name] = {
            "questions": questions,
            "answers": answers,
        }
...
def build_train_and_val_datasets(module_name_to_arrays, params):
    ...
    for module_name in module_name_to_arrays:
        ...
        val_ds = (
            tf.data.Dataset.from_tensor_slices((val_questions, val_answers))
    ...
    train_ds = (
        tf.data.Dataset.from_tensor_slices((train_questions, train_answers))
        .shuffle(num_train)
        .batch(params.batch_size)
        .prefetch(buffer_size=tf.data.experimental.AUTOTUNE)
    )
