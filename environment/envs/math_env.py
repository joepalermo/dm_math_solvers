...
class MathEnv(gym.Env):
    ...
    def __init__(self, problem_filepaths, num_problems_per_module=int(1e7)):
        ...
        for module_type in self.train:
            ...
            for difficulty in self.train[module_type]:
                ...
                self.val[module_type][difficulty] = self.train[module_type][difficulty][:num_val]
                self.train[module_type][difficulty] = self.train[module_type][difficulty][num_val:]
                assert len(self.train[module_type][difficulty]) + len(self.val[module_type][difficulty]) == num_examples
        # load test data
        # TODO
    ...
    def reset_with_specific_problem(self, module_type, difficulty, problem_index, train=True):
        self.problem_statement, self.answer = self.train[module_type][difficulty][problem_index]
    ...
    def reset_by_module_and_difficulty(self, module_type, difficulty, train=True):
        self.problem_statement, self.answer = sample(self.train[module_type][difficulty], 1)[0]
        self.compute_graph = ComputeGraph(self.problem_statement)
        return self.problem_statement
